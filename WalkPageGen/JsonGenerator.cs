...
namespace WalkPageGen
{
    public static class JsonGenerator
    {
        public static string CreateJson(List<IEvent> events, bool useMongoDateFormat)
        {
            ...
            foreach(var ev in events.Where(w => w.IsRoute))
            {
                if (needComma)
                    sb.AppendLine(",");
        ...
        private static void AppendEvent(IEvent ev, StringBuilder sb, bool useMongoDbFormat)
        {
            ...
            sb.Append("\t\t\"length\": ").Append(ev.Length).AppendLine(",");
            ...
            sb.Append("\t\t\"walkTime\": ").Append(ev.Duration).AppendLine(",");
            ...
            sb.Append("\t\t\"fuelCost\": ").Append(ev.FuelCost).AppendLine();
            sb.Append("\t}");
        }
    }
}
